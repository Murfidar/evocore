...
from evocore.optimizers.cmaes import CategoricalDistributionState, IntegerMarginDistribution


def test_integer_margin_keeps_probability_mass_inside_bounds() -> None:
    margin = IntegerMarginDistribution(low=0, high=3, min_probability=0.10)
...
def test_categorical_state_updates_toward_better_category() -> None:
    state = CategoricalDistributionState(categories=(0, 1, 2), learning_rate=0.5)
