...
def run_child_optimizer(
    engine: _ChildOptimizer, seed: int, evaluator: Evaluator
) -> OptimizationResult:
    """Run one child optimizer with a derived seed for process-pool execution."""
    return engine._copy_with_seed(seed).run(evaluator)


class GeneticAlgorithmMultiRunMixin:
    """Seed derivation, optimizer copying, and multi-run execution."""
    ...
    def run_multiple(
        ...
        Returns:
            Multi-run result sorted by descending best score.
...
__all__ = ["GeneticAlgorithmMultiRunMixin", "run_child_optimizer"]
